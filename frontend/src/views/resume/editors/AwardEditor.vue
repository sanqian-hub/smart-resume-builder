<template>
  <div class="editor-section">
    <div v-for="(item, idx) in list" :key="idx" class="award-block">
      <div v-if="list.length > 1" class="block-header">
        <span class="block-index">荣誉奖项 {{ idx + 1 }}</span>
      ...
      <div class="form-grid cols-3">
        ...
        <div class="form-item">
          <label>获奖时间</label>
          <div class="custom-select" :class="{ open: openDropdown === dKey(idx) }">
            <div class="select-display" @click="toggleDropdown(dKey(idx), item.date)">
              <span class="select-text" :class="{ placeholder: !item.date }">{{ item.date || '' }}</span>
              <button v-if="item.date" class="select-clear-btn" @click.stop="item.date = ''; openDropdown = ''; emitChange()">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18"/><path d="M6 6l12 12"/></svg>
              </button>
            </div>
            <div v-if="openDropdown === dKey(idx)" class="select-options date-options">
              <div class="date-picker-panel">
                <div class="dp-nav">
                  <button class="dp-nav-btn" @click.stop="changeYear(dKey(idx), -1)"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M15 18l-6-6 6-6"/></svg></button>
                  <span class="dp-year">{{ pickerYear[dKey(idx)] }}</span>
                  <button class="dp-nav-btn" @click.stop="changeYear(dKey(idx), 1)"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M9 18l6-6-6-6"/></svg></button>
                </div>
                <div class="dp-months">
                  <div v-for="m in months" :key="m" class="dp-month" :class="{ active: item.date === fmtDate(dKey(idx), m) }" @click="pickDate(item, 'date', dKey(idx), m)">{{ m }}月</div>
                </div>
              </div>
            </div>
          </div>
    ...
    <div v-if="list.length === 0" class="empty-hint">暂无荣誉奖项，点击下方添加</div>
...
const emptyItem = { name: '', date: '', content: '' }
...
function dKey(idx) { return 'ad-' + idx }
...
.form-grid.cols-3 .form-item:last-child { max-width: 160px; }
