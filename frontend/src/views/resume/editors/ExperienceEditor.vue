<template>
  <div class="editor-section">
    <div v-for="(item, idx) in list" :key="idx" class="exp-block">
      <div v-if="list.length > 1" class="block-header">
        <span class="block-index">工作经历 {{ idx + 1 }}</span>
      ...
      <div class="form-grid cols-3">
        <div class="form-item">
          ...
          <div class="input-wrap">
            <input v-model="item.company" @input="emitChange" />
            <button v-if="item.company" class="input-clear-btn" @click="item.company = ''; emitChange()">
      ...
      <div class="form-grid cols-3" style="margin-top: 4px;">
        ...
        <div class="form-item">
          <label>岗位名称</label>
          <div class="input-wrap">
            <input v-model="item.position" @input="emitChange" />
            <button v-if="item.position" class="input-clear-btn" @click="item.position = ''; emitChange()">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18"/><path d="M6 6l12 12"/></svg>
            </button>
          </div>
        </div>
    ...
    <div v-if="list.length === 0" class="empty-hint">暂无工作经历，点击下方添加</div>
...
const emptyItem = {
  company: '', department: '', position: '', city: '',
...
.select-display {
  display: flex; align-items: center; justify-content: space-between; padding: 0 10px; font-size: 13px; height: 34px;
  position: relative;
  border: 1px solid var(--border); border-radius: 6px;
