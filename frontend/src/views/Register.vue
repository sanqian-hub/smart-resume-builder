    ...
    <div class="right-panel">
      <div class="right-panel-inner">
        <div class="form-box">
          ...
          <form @submit.prevent="handleRegister" class="auth-form">
            <div class="form-row">
              <div class="field">
                ...
                <input id="username" v-model="form.username" type="text" placeholder="你的昵称" autocomplete="name" @focus="isTyping = true" @blur="isTyping = false" required />
              </div>
              <div class="field">
                <label for="account">账号</label>
                <input id="account" v-model="form.userAccount" type="text" placeholder="至少4位" autocomplete="username" @focus="isTyping = true" @blur="isTyping = false" required />
              </div>
            </div>
            ...
            <div class="field">
              ...
              <div class="pw-wrap">
                <input id="password" v-model="form.userPassword" :type="showPassword ? 'text' : 'password'" placeholder="至少8位" autocomplete="new-password" @focus="isTyping = true" @blur="isTyping = false" required />
            ...
            <div class="form-row">
              <div class="field">
                ...
                <input id="phone" v-model="form.phone" type="tel" placeholder="选填" autocomplete="tel" @focus="isTyping = true" @blur="isTyping = false" />
              </div>
              <div class="field">
                <label for="email">邮箱</label>
                <input id="email" v-model="form.email" type="email" placeholder="选填" autocomplete="email" @focus="isTyping = true" @blur="isTyping = false" />
              </div>
            </div>
          ...
          <p class="signup-link">
            已有账号？<router-link :to="loginLink">立即登录</router-link>
          </p>
        </div>
...
const form = reactive({ username: '', userAccount: '', userPassword: '', phone: '', email: '' })
...
function validateRegisterForm() {
  ...
  if (illegalAccountChars.test(userAccount)) {
    return '账号包含非法字符'
  }
  if (!password) {
    return '密码不能为空'
